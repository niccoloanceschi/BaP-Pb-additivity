...
The repo implements the BaP-Pb toxicant-additivity analysis presented in the manuscript. The analysis uses a convex-mixture model \( F(\cdot \mid d) = (1-\lambda(d))F_0 + \lambda(d)F_\infty, \) for ordinal damage-score distributions.
Hhere, \(F_0\) and \(F_\infty\) represent the unexposed and maximal-effect profiles, respectively, and the dose dependence is captured by the monotone weight function \(\lambda(d)\).
...
Each null model induces an additive baseline surface \(\lambda_{12}^{(o)}(d_1,d_2)\) based on the corresponding single-exposure fits.
...
| `Model_fit.R` | Fitting of the single-exposure models and joint radial-spline surface |
...
5. **Model fitting** - fits the single-exposure models and joint-exposure radial-spline surface.
